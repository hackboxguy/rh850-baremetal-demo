...
- It ports `bios-scripts/bios-ver-11.txt` into C using generated profile data.
...
- Generated profile data is optimized:
  ...
  - shared init-op blocks across related profiles
- `panels.json` plus versioned `app/983_manager/edid/*.bin` assets are the
  active source of truth for selectable profiles; `bios-ver-11.txt` remains the
  reference when there is doubt.
- `tools/check_983_manager.py` is the host-side safety net after generator,
  manifest, or EDID changes. It verifies:
  - selector uniqueness
  - EDID asset validity
